...
PaciNomePorId: async (req, res) => {
    const { paci_id } = req.params; // O paci_id deve ser passado como parâmetro na URL

    try {
      const paciNome = await PacienteModel.PaciNomePorId(paci_id);

      if (paciNome) {
        res.json({ paci_nome: paciNome });
      } else {
        res.status(404).json({ error: 'Paciente não encontrado.' });
      }
    } catch (error) {
      res.status(500).json({ error: 'Erro ao buscar o paciente.', details: error });
    }
  },
