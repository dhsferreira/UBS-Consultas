...
const Consultas = () => {
  ...
  const fetchConsultas = async (selectedDate?: string) => {
    try {
      if (ubsId !== null) {
        ...
        let url = `http://192.168.0.102:3000/api/consultas/ubs/${ubsId}/area/${areaNome}`;
        if (selectedDate) {
  ...
  const handleExames = async (consulta: Consulta) => {
    try {
      ...
      const response = await axios.get(`http://192.168.0.102:3000/api/BuscarNome/${consulta.paci_nome}`);
