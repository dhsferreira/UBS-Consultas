...
const Consultas = ({ route }) => {
  ...
  useEffect(() => {
    const fetchPaciNome = async () => {
      try {
        const response = await axios.get(`http://10.47.7.48:3000/api/Nome/${paciId}`);
  ...
  const fetchUbsNome = async (ubsId) => {
    try {
      const response = await axios.get(`http://10.47.7.48:3000/api/Ubs/${ubsId}/nome`);
  ...
  const adicionarReceita = async () => {
    try {
      const response = await axios.post('http://10.47.7.48:3000/api/criarReceita', {
        ubs_id: ubsId,
        medi_id: mediId,
        paci_id: paciId,
