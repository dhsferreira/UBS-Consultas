...
const Consultas = ({ route }) => {
  ...
  useEffect(() => {
    const fetchPaciNome = async () => {
      try {
        const response = await axios.get(`http://192.168.137.1:3000/api/Nome/${paciId}`);
  ...
  const fetchUbsNome = async (ubsId) => {
    try {
      const response = await axios.get(`http://192.168.137.1:3000/api/Ubs/${ubsId}/nome`);
  ...
  const adicionarReceita = async () => {
    try {
      const response = await axios.post('http://192.168.137.1:3000/api/criarReceita', {
        ubs_id: ubsId,
        medi_id: mediId,
        paci_id: paciId,
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.form}>
        <TextInput
          style={styles.input}
          placeholder={ubsNome || "Carregando..."}
          value={ubsNome}
          editable={false}
        />
        <TextInput
          style={styles.input}
          placeholder={mediNome || "Carregando..."}
          value={mediNome}
          editable={false}
        />
        <TextInput
          style={styles.input}
          placeholder={paciNome || "Carregando..."}
          value={paciNome}
          editable={false}
        />
        <TextInput
